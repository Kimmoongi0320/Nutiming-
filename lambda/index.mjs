...
- 오메가3 + 비타민E: 항산화 시너지
- 칼슘 + 비타민D: 칼슘 흡수 극대화
- 아연 + 구리: 균형 유지 (아연 고용량 복용 시 구리 고갈 예방)
...
[특수 상황]
- 임산부/수유부: 엽산, 철분 중요성 강조
- 공복 불량 영양제: 아연, 철분, 마그네슘(고용량), 어류 오메가3

중요 규칙:
- schedule의 각 배열에는 반드시 사용자가 입력한 영양제 이름을 그대로 사용하세요. 임의로 변경하거나 번역하지 마세요.
    ...
    schedule: {
      ...
      properties: {
        morning_empty: {
          type: 'array',
          items: { type: 'string' },
          description: '아침 공복에 복용 (기상 후 30분 이내)',
        },
        morning_with_food: {
          type: 'array',
          items: { type: 'string' },
          description: '아침 식사와 함께 복용',
        },
        afternoon: {
          type: 'array',
          items: { type: 'string' },
          description: '점심 식사와 함께 복용',
        },
        evening_with_food: {
          type: 'array',
          items: { type: 'string' },
          description: '저녁 식사와 함께 복용',
        },
        before_bed: {
          type: 'array',
          items: { type: 'string' },
          description: '취침 30-60분 전 복용',
        },
      },
  ...
  try {
    ...
    if (!Array.isArray(supplements) || supplements.length === 0) {
      return {
        ...
        body: JSON.stringify({ error: '영양제 목록이 비어있습니다.' }),
      };
    }
